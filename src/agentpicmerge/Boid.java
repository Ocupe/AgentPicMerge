...
public class Boid {
	...
	PApplet p;
	  ...
	  public Boid(PApplet p_ ,Vec2D l, float ms, float mf) {
		p = p_;
	    loc=l;
	    acc = new Vec2D();
	  ...
	  void render() {
	    ...
	    float theta = vel.heading() + p.radians(90f);
	    p.fill(175);
	    p.stroke(0);
	    p.pushMatrix();
	  ...
	  // Wraparound
	  void borders() {
	    if (loc.x < -r) loc.x = p.width+r;
	    if (loc.y < -r) loc.y = p.height+r;
	    if (loc.x > p.width+r) loc.x = -r;
	    if (loc.y > p.height+r) loc.y = -r;
	  }
